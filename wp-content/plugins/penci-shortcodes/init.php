...
define( 'PENCI_SOLEDAD_SHORTCODE_PERFORMANCE', '6.2' );
define( 'PENCI_SOLEDAD_SHORTCODE_URL', plugin_dir_url( __FILE__ ) );
...
if ( ! function_exists( 'penci_lazy_toolbar_link' ) ) {
	function penci_lazy_toolbar_link( $wp_admin_bar ) {
		...
		$var = ( is_ssl() ? 'https://' : 'http://' ) . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'];
		$clear_url = add_query_arg( 'clear_pencilazy_css', 'true', $var );
		...
		if ( current_user_can( 'manage_options' ) && ( get_theme_mod( 'penci_speed_remove_css' ) || get_theme_mod( 'penci_speed_optimize_css' ) ) ) {
			...
			if ( is_multisite() ) {
				...
				$clear_url = add_query_arg( 'clear_pencilazy_css_single', 'true', $var );
...
add_filter( 'intermediate_image_sizes_advanced', function( $sizes ) {
	if ( ! get_theme_mod( 'penci_disable_unnecessary_image_sizes' ) ) {
		return $sizes;
	}

	// Map theme mods to their corresponding image sizes
	$image_map = [
		'penci_dthumb_1920_auto'  => 'penci-single-full',
		'penci_dthumb_1920_800'   => 'penci-slider-full-thumb',
		'penci_dthumb_1170_auto'  => 'penci-full-thumb',
		'penci_dthumb_1170_663'   => 'penci-slider-thumb',
		'penci_dthumb_780_516'    => 'penci-magazine-slider',
		'penci_dthumb_585_390'    => 'penci-thumb',
		'penci_dthumb_585_auto'   => 'penci-masonry-thumb',
		'penci_dthumb_585_585'    => 'penci-thumb-square',
		'penci_dthumb_480_650'    => 'penci-thumb-vertical',
		'penci_dthumb_263_175'    => 'penci-thumb-small',
	];

	foreach ( $image_map as $theme_mod => $image_size ) {
