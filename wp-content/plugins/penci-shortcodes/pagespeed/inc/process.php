	...
	public function setup() {
		if ( $this->should_process() ) {
			...
			add_action( 'template_redirect', function () {
				...
				if ( \is_page() && get_post_meta( get_queried_object_id(), 'penci_critical_disable', true ) ) {
					return false;
				}
...
function hpp_get_embed_video_url( $str ) {
	$r    = array();
	$size = get_theme_mod( 'penci_youtube_thumbnail_size', 'maxresdefault' );
	//youtube
	if ( strpos( $str, 'youtube.com/watch?v=' ) !== false ) {
		...
		$r['thumb'] = 'https://i.ytimg.com/vi/' . $id . '/' . $size . '.jpg';
