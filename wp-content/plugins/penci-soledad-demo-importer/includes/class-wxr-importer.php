...
class WXR_Importer extends WP_Importer {
	...
	public function __construct( $options = array() ) {
		...
		$this->options = wp_parse_args( $options, array(
			...
			'pages'        				=> false,
			'header_footer_only'        => false,
			'content_only_pages'        => false,
			'only_import_posts'         => false,
...
//		libxml_use_internal_errors( true );
			...
			if ( '' === $reader->namespaceURI ) {
				...
				switch ( $reader->localName ) {
					...
					case 'item':
						...
						if ( $this->options['header_footer_only'] ) {
							// Skip non-header/footer posts
							if ( ! in_array( $parsed['data']['post_type'], array( 'penci-block', 'penci_builder', 'attachment' ), true ) ) {
								$reader->next();
								break;
							}
						} elseif ( $this->options['only_import_posts'] ) {
							if ( ! in_array( $parsed['data']['post_type'], array( 'post', 'attachment' ), true ) ) {
								$reader->next();
								break;
							}
						} elseif ( $this->options['only_import_products'] ) {
							if ( ! in_array( $parsed['data']['post_type'], array( 'product', 'attachment' ), true ) ) {
								$reader->next();
								break;
							}
						} elseif ( $this->options['content_only_pages'] || $this->options['pages'] ) {
							if ( ! in_array( $parsed['data']['post_type'], array( 'page', 'attachment' ), true ) ) {
								$reader->next();
								break;
							}
						}
	...
	/**
	 * Replace any image URL that belongs to the demo hosts with a single placeholder.
	 *
	 * Handles serialized PHP, JSON, nested structures, escaped slashes, and many common edge cases.
	 *
	 * @param mixed $value
	 * @return mixed
	 */
	protected function replace_demo_image_domains_in_value( $value ) {
		$hosts = array(
		...
		$placeholder = 'https://placehold.jp/30/ccc/000/1920x1080.png?text=Soledad+Placeholder+Image';

		return $this->recursive_replace_image_urls( $value, $hosts, $placeholder );
	}

	/**
	 * Recursively replace image urls inside arrays/objects/strings (including serialized/json).
	 *
	 * @param mixed $data
	 * @param array $hosts   hostnames to search for (without protocol)
	 * @param string $placeholder full placeholder URL (with protocol)
	 * @return mixed
	 */
	protected function recursive_replace_image_urls( $data, array $hosts, $placeholder ) {
		// Arrays
		if ( is_array( $data ) ) {
			foreach ( $data as $k => $v ) {
				$data[ $k ] = $this->recursive_replace_image_urls( $v, $hosts, $placeholder );
			}
			return $data;
		}
		...
		foreach ( $hosts as $host ) {
			...
			// Match images from demo host
			$pattern = '#https?://(?:www\.)?' . $host_esc . '/[^\s\'"<>]+?\.(jpg|jpeg|png|gif|webp)(?:\?[^\s\'"<>]*)?#i';
	
			$out = preg_replace_callback( $pattern, function ( $matches ) {
				$url = $matches[0];
	
				$width  = 1920; // fallback default
				$height = 1080;
	
				// Try filename-based detection: e.g., "image-800x600.jpg"
				if ( preg_match( '/(\d+)[xX](\d+)\.(jpg|jpeg|png|gif|webp)$/i', $url, $m ) ) {
					$width  = (int) $m[1];
					$height = (int) $m[2];
				} else {
					// Try getimagesize() on remote file (may be slow)
				...
				// Build placeholder URL with dimensions
				return "https://placehold.jp/30/ccc/000/{$width}x{$height}.png?text=Soledad+Placeholder+Image";
			}, $out );
		}
	
		// If something changed, return it immediately
		if ( $out !== $data ) {
			return $out;
		}
		...
		if ( function_exists( 'maybe_unserialize' ) ) {
			...
			if ( $maybe !== $data ) {
				$replaced = $this->recursive_replace_image_urls( $maybe, $hosts, $placeholder );
		...
		if ( json_last_error() === JSON_ERROR_NONE && ( is_array( $decoded ) || is_object( $decoded ) ) ) {
			$replaced = $this->recursive_replace_image_urls( $decoded, $hosts, $placeholder );
	...
	protected function parse_meta_node( $node ) {
		...
		$penci_skip_import_attachment = apply_filters( 'penci_skip_import_attachment', true );

		if ( $penci_skip_import_attachment ) {
			$value = $this->replace_demo_image_domains_in_value( $value );
		}
	...
	protected function process_attachment( $post, $meta, $remote_url ) {
		...
		// Keep original URL (with query string) for fetching
		$original_url   = $remote_url;
		// Normalized URL (without query/fragment) for duplicate check and remap
		$normalized_url = strtok( $remote_url, '?#' );
	
		// if the URL is absolute, but does not contain address, then upload it assuming base_site_url
		if ( preg_match( '|^/[\w\W]+$|', $remote_url ) ) {
			$original_url   = rtrim( $this->base_url, '/' ) . $remote_url;
			$normalized_url = strtok( $original_url, '?#' );
		}
	
		// Check for duplicate attachment
		$existing_id = attachment_url_to_postid( $normalized_url );
		if ( $existing_id ) {
			$this->url_remap[ $normalized_url ] = wp_get_attachment_url( $existing_id );
			if ( substr( $normalized_url, 0, 8 ) === 'https://' ) {
				$insecure_url                     = 'http' . substr( $normalized_url, 5 );
				$this->url_remap[ $insecure_url ] = wp_get_attachment_url( $existing_id );
			}
			return $existing_id;
		}
	
		// Fetch remote file with full original URL
		$upload = $this->fetch_remote_file( $original_url, $post );
		...
		// Map normalized URL to local one
		$this->url_remap[ $normalized_url ] = $upload['url'];
	...
	protected function process_post( $data, $meta, $comments, $terms ) {
		...
		$penci_skip_attachment = apply_filters( 'penci_skip_import_attachment', true );
		$placeholder_image_id  = get_option( 'penci_soledad_placeholder_img' );

		if ( $penci_skip_attachment ) {
			$data['post_content'] = penci_replace_demo_images_in_content( $data['post_content'] );
		}
		...
		if ( 'attachment' === $postdata['post_type'] ) {
			...
			if ( $penci_skip_attachment && $placeholder_image_id ) {
				$post_id = $placeholder_image_id;
			} else {
	...
	protected function repair_serialized_string( $value ) {
	...
	protected function process_post_meta( $meta, $post_id, $post ) {
		...
		foreach ( $meta as $meta_item ) {
			...
			if ( $key ) {
				...
				if ( ! $value ) {
					$raw_value = $meta_item['value'];

					// 🔧 Elementor _elementor_data safe handling
					if ( '_elementor_data' === $key && is_string( $raw_value ) ) {
						$raw_value = wp_slash( wp_unslash( $raw_value ) ); // save raw
					}
					// 🔧 Serialized meta fix
					elseif ( is_string( $raw_value ) && false !== strpos( $raw_value, 's:' ) ) {
						$raw_value = $this->repair_serialized_string( $raw_value );
					}
				...
				if ( '_thumbnail_id' === $key && $penci_skip_attachment && $placeholder_image_id ) {
					update_post_meta( $post_id, '_thumbnail_id', $placeholder_image_id );
				}

				if ( '_format_gallery_images' === $key && $penci_skip_attachment && $placeholder_image_id ) {
					update_post_meta( $post_id, '_format_gallery_images', array( $placeholder_image_id ) );
				}
			}
		}
	...
	protected function process_term( $data, $meta ) {
		...
		if ( ! empty( $data['parent'] ) ) {
			...
			elseif ( $parent_id = $this->term_exists( array(
	...
	protected function fetch_remote_file( $url, $post ) {
		// extract the file name from the URL path (strip query string / fragment)
		$path = parse_url( $url, PHP_URL_PATH );
		$file_name = $path ? basename( $path ) : '';
		if ( empty( $file_name ) ) {
			$file_name = 'imported-file-' . time();
		}
		...
		// fetch the remote url and write it to the placeholder file (preserve full URL for request)
		...
		// ensure the filesystem info is fresh
		if ( function_exists( 'clearstatcache' ) ) {
			clearstatcache( false, $upload['file'] );
		}
	
		$filesize = @filesize( $upload['file'] );
	
		$content_length = wp_remote_retrieve_header( $response, 'content-length' );
		if ( $content_length && $filesize !== (int) $content_length ) {
			if ( file_exists( $upload['file'] ) ) {
				@unlink( $upload['file'] );
			}
