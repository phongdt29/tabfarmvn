...
class Penci_Demo_Widgets_Importer {
	/**
	 * Replace demo image URLs inside widget option data.
	 *
	 * @param array $widget_instances The widget option array (all instances).
	 * @return array Updated array with demo images replaced.
	 */
	protected function replace_demo_images_in_widgets( $widget_instances ) {
		...
		foreach ( $widget_instances as $key => $instance ) {
			...
			array_walk_recursive( $instance, function ( &$value, $field ) {
				if ( ! is_string( $value ) || $value === '' ) {
					return;
				}

				// Run the same replacement function used for post_content
				$value = penci_replace_demo_images_in_content( $value );
			} );
				...
				if ( ! $fail ) {
					...
					$single_widget_instances[] = $widget; // add it
						...
						if ( $penci_skip_attachment ) {
							$single_widget_instances = $this->replace_demo_images_in_widgets( $single_widget_instances );
						}
