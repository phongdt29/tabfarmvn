...
class HeaderBuilder {
	...
	public static function desktop_header_element() {
		$elements = array(
			...
			'pb_vertical_menu'   => esc_attr__( 'Horizontal Scroll Menu', 'soledad' ),
	...
	public function add_edit_link( $actions, $post ) {
		...
		if ( in_array( $post->post_type, $export_posttype ) ) {
			...
			$preview_url       = add_query_arg( $arg_query, home_url() );
			...
			$actions['penci-builder-preview-link'] = '<a href="' . esc_url( $preview_url ) . '" class="penci-builder-preview-link">' . __( 'Preview', 'soledad' ) . '</a>';
		}
