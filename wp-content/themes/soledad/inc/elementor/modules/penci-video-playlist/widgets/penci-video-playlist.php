...
class PenciVideoPlaylist extends Base_Widget {
	...
	protected function register_controls() {
		...
		$this->add_control( 'video_type', array(
			...
			'options' => array(
				...
				'youtube_channel'  => esc_html__( 'Youtube Channel', 'soledad' ),
				'local' 		   => esc_html__( 'Local Videos', 'soledad' ),
			),
		) );

		$this->add_control( 'video_channel_notice', array(
			'type'            => Controls_Manager::RAW_HTML,
			'raw'             => '<div class="penci-admin-notice">' . __( 'Note: You can get the Youtube Channel ID by visit <a target="_blank" href="https://www.streamweasels.com/%20tools/youtube-channel-id-and-%20user-id-convertor/">this link</a>.', 'soledad' ) . '</div>',
			'content_classes' => 'elementor-descriptor',
			'condition'       => [ 'video_type' => 'youtube_channel' ],
		) );
		...
		$this->add_control( 'youtube_channel_id', array(
			'label'       => __( 'Youtube Channel ID', 'soledad' ),
			'type'        => Controls_Manager::TEXT,
			'condition'   => [ 'video_type' => 'youtube_channel' ],
			'description' => __( 'Enter Youtube Channel ID', 'soledad' ),
		) );

		$this->add_control( 'yplaylist_limit', array(
			'label'     => __( 'Limit Items on Youtube', 'soledad' ),
			'type'      => Controls_Manager::NUMBER,
			'default'   => '5',
			'condition' => [ 'video_type' => ['youtube_playlist','youtube_channel'] ],
		) );
	...
	protected function render() {
		...
		if ( ( ! $settings['videos_list'] && $settings['video_type'] == 'custom' ) || ( $settings['video_type'] == 'youtube_playlist' && ! $settings['yplaylist_id'] ) || ( $settings['video_type'] == 'youtube_channel' && ! $settings['youtube_channel_id'] ) ) {
			return;
		}
        ...
        <div class="<?php echo esc_attr( $css_class ); ?>">
            ...
            <div class="penci-block_content">
				...
				if ( $settings['video_type'] == 'youtube_channel' && $settings['youtube_channel_id'] ) {
					$videos_list = \Penci_Video_List::get_playlist_channel_ids( $settings['youtube_channel_id'], $settings['yplaylist_limit'] );
				}
