...
class PenciButton extends Base_Widget {
	...
	protected function render() {
		...
		$link_attr = '';
		$link_attr_data = $settings['link']['custom_attributes'] ?? '';
		if ( $link_attr_data ) {
			$link_attrs = array_filter( array_map( 'trim', explode( ',', $link_attr_data ) ) );
			foreach ( $link_attrs as $attr ) {
				$attr_parts = explode( '|', $attr, 2 );
				if ( count( $attr_parts ) === 2 ) {
					$link_attr .= ' ' . esc_attr( $attr_parts[0] ) . '="' . esc_attr( $attr_parts[1] ) . '"';
				}
			}
		}
		$button_id_attr = '';
